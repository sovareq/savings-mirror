...
pub struct DailyBucket {
    ...
    /// Totaal output-tokens van model-replies, gesommeerd uit Claude-Code transcripts.
    #[serde(default)]
...
fn build_report_uncached() -> Result<CavemanReport> {
        ...
        })
    {
        ...
        for line in reader.lines().map_while(Result::ok) {
            ...
            day.out_tokens_total += out_tokens;
            ...
            mb.out_tokens_total += out_tokens;
        }
    }
    ...
    for (d, b) in &by_day {
        ...
        cumulative.out_tokens_total += b.out_tokens_total;
        if d.num_days_from_ce() >= week_cut {
            ...
            last_7d.out_tokens_total += b.out_tokens_total;
        }
    }
    last_7d.finalise();
    cumulative.finalise();
...
mod tests {
    ...
    #[test]
    fn out_tokens_total_is_set_from_sample_build() {
        // A single 100k-token Haiku call → bucket.out_tokens_total == 100_000.
        let b = single_call("claude-3-5-haiku", 100_000, "full");
        assert_eq!(b.out_tokens_total, 100_000);
        assert!(b.out_tokens_total > 0);
    }
}
